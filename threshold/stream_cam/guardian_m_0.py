...
from threshold.stream_cam.iterq import IterQueue
#db
...
#------------------------------------------------------------------------------
                #1-run process according to mode
#****************************************************************************
		#for cam 1
		#for model 1
		if self.db.cam1_active is True: 
			...
			if self.db.model1.id == 1:
				print("cam active and model =")
				print(self.db.model1.id)
				self.cam_ids.append(1)
				self.p1 = Process(target=self.get_frame)
				self.p1_alive =True
				self.p1.start()
				if self.db.cam1_active == False:
					self.p1.join()
					self.p1_alive = False
			#for model 2
			if self.db.model1.id == 2:
				...
				self.p1 = Process(target=self.get_seg)
				self.p1_alive =True
				self.p1.start()
				if self.db.cam1_active == False:
					self.p1.join()
			...
			if self.db.model1.id == 3:
				...
				self.p1 = Process(target=self.get_pose)
				self.p1_alive =True
				self.p1.start()
				if self.db.cam1_active == False:
					self.p1.join()
...
#------------------------------------------------------------------------------
		...
		if self.db.cam2_active is True:
			if self.db.model2.id == 0:
				self.cam_ids.append(1)
				self.p2 = Process(target=self.get_frame, args=(self.q2))
				self.p2.start()
				if self.db.cam2_active == False:
					self.p2.join()
		...
		if self.db.cam2_active is True:
			if self.db.model2.id == 1:
				self.cam_ids.append(2)
				self.p2 = Process(target=self.get_pose, args=(self.q2))
				self.p2.start()
				if self.db.cam2_active == False:
					self.p2.join()
		#for model 3
...
#------------------------------------------------------------------------------
		...
		if self.db.cam3_active is True:
			if self.db.model3.id == 1:
				self.cam_ids.append(1)
				self.p3 = Process(target=self.get_frame, args=(self.q3))
				self.thread3.start()
				if self.db.cam3_active == False:
					self.p3.join()
		#for model 2
		if self.db.cam3_active is True:
			if self.db.model3.id == 2:
				self.cam_ids.append(2)
				self.p3 = Process(target=self.get_pose, args=(self.q3))
				self.thread3.start()
				if self.db.cam3_active == False:
					self.p3.join()
...
#------------------------------------------------------------------------------
		...
		if self.db.cam4_active is True:
			if self.db.model4.id == 0:
				self.cam_ids.append(1)
				self.p4 = Process(target=self.get_frame, args=(self.q4))
				self.p4.start()
				if self.db.cam4_active == False:
					self.p4.join()
		#for model 2
		if self.db.cam4_active is True:
			if self.db.model4.id == 1:
				self.cam_ids.append(2)
				self.p4 = Process(target=self.get_pose, args=(self.q4))
				self.p4.start()
				if self.db.cam4_active == False:
					self.p4.join()
		#for model 3
...
#------------------------------------------------------------------------------
		...
		if self.db.cam5_active is True:
			if self.db.model5.id == 0:
				self.cam_ids.append(1)
				self.p5 = Process(target=self.get_frame, args=(self.q5))
				self.p5.start()
				if self.db.cam5_active == False:
					self.p5.join()
		#for model 2
		if self.db.cam5_active is True:
			if self.db.model5.id == 1:
				self.cam_ids.append(2)
				self.p5 = Process(target=self.get_pose, args=(self.q5))
				self.p5.start()
				if self.db.cam5_active == False:
					self.p5.join()
		#for model 3
	...
	def get_frame(self):
		...
		while True:
			...
			try:
				...
				results = self.model1.predict(resized,show_labels=True, show_conf=True,device="cpu") 
				#results=image
			except Exception as e:
				print(e)
				continue
