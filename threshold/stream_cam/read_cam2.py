...
class launch_module(object):
	def __init__(self,ob_req):
		...
		self.chosen_model = self.db.model1.id
		#load models
	...
	def get_frame(self): #self.vido.read()
		while True:
			...
			try:
				...
				match self.chosen_model:
					case 1:
						results = self.model(image,stream=True)
					case 2:
						results = self.model2(image,stream=True)
					case 3:
						results = self.model3(image,stream=True)
					case _:
						results = self.video.read()
				#results=image
